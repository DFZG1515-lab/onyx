...
/** Anillo de uso del ingreso: comprometido en pizarra, gastado en tinta, resto en regla. */
export function Anillo({ comprometido, gastado, etiqueta, animar }: Props) {
  ...
  return (
    <svg className="anillo" width="84" height="84" viewBox="0 0 84 84" role="img" aria-label={`${etiqueta}: ${porcentaje} por ciento`}>
      ...
      <circle cx="42" cy="42" r={RADIO} fill="none" stroke="var(--slate)" strokeWidth="6" strokeDasharray={`${c} ${LARGO}`} transform="rotate(-90 42 42)" />
      <circle cx="42" cy="42" r={RADIO} fill="none" stroke="var(--ink)" strokeWidth="6" strokeDasharray={`${g} ${LARGO}`} strokeDashoffset={-c} transform="rotate(-90 42 42)" />
