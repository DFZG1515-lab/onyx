...
- 8 sep 2026: categorías editables (crear, renombrar, palabras clave, borrar pasando gastos a Otros), ingresos extra por quincena (suman al disponible, no al ritmo; tabla `ingresos`), gastos compartidos («Dividir entre» en la captura guarda tu parte y crea una deuda; tabla `deudas`; «Ya me pagaron» la convierte en ingreso extra), historial que abre los movimientos de esa quincena, primer cargo ajustable en MSI (`primerPagoCicloId`), pantalla de error si IndexedDB no abre y límite de errores de React. `pagosHechos` se quitó del modelo. Dexie en v3. Respaldo en v2 (lee v1). Netlify corre `npm test && npm run build`.
- El Resumen está partido en `src/pantallas/resumen/`: `useResumen.ts` calcula, los demás archivos dibujan.
